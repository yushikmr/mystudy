...
class C20Patient:
    def __init__(self, record_path: Path) -> None:
        ...
        self._df = self.record.to_dataframe().iloc[:: self.samplingrate]
    ...
    def _get_labelids(self, record_path):
        ...
        self._labelids: List[str] = [
            lid for lid in _labelids if lid in self._labelsdict.keys()
        ]
    ...
    def Ynp(self):
        ...
        for label in self.get_label:
            ind = self.labelnames.index(label)
            ynp[ind] = 1
        return ynp
...
class C20:
    ...
    def _get_valid_patients(self, patients: List[C20Patient]) -> List[C20Patient]:
        valid_patients = [
            patient for patient in patients if patient.get_labelID() in self._valid_ids
        ]
...
class C20DatasetOnMemory(Dataset):
    def __init__(
        self, valid_files_csv, use_channels: list, sample_size, num_classes
    ) -> None:
        """"""
    ...
    def _create_tensor(self):
        ...
        for i in range(self.patients_files.shape[0]):
            ...
            p = C20Patient(f)
            X_list.append(
                p.toTensorX[self.use_channels, :].reshape(
                    1, self.num_channels, self.sample_size
                )
            )
